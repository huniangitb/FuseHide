...
void* gOriginalUnlinkLibc = nullptr;
void* gOriginalRmdirLibc = nullptr;
void* gOriginalRenameLibc = nullptr;
...
std::string HiddenPathPolicy::GetRedirectTarget(uint32_t uid, const std::string& fusePath) {
    ...
    for (const auto& rule : config->redirectRules) {
        ...
        if (pkgMatch) {
            ...
            if (hasStar) {
                if (fusePath.starts_with(pat)) {
                    std::string tail = fusePath.substr(pat.length());
                    if (tail.empty() || tail[0] == '/') {
                        std::string res = rule.target;
                        if (!res.ends_with('/') && !tail.empty() && tail[0] == '/') {
                            res += tail;
                        } else if (res.ends_with('/') && !tail.empty() && tail[0] == '/') {
                            res += tail.substr(1);
                        } else {
                            res += tail;
                        }
                ...
                if (fnmatch(rule.pattern.c_str(), fusePath.c_str(), FNM_PATHNAME) == 0 || fusePath == rule.pattern) {
                    return rule.target;
                }
            }
        }
